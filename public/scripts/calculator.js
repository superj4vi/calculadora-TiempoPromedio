...
function processHours(input) {
  ...
  for (let line of lines) {
    ...
    if (!validateTimeFormat(line)) {
      throw new Error(
        `Formato inválido en la línea: "${line}". Usa HH:MM:SS, HH:MM, o horas (ej. 8, 7.5)`,
      );
    }
...
function displayResults(totalSeconds, processedHours) {
  const { hours, minutes, seconds } =
    secondsToHoursMinutesSeconds(totalSeconds);
  ...
  const {
    ...
    minutes: avgMinutes,
    seconds: avgSeconds,
  } = secondsToHoursMinutesSeconds(Math.round(averageSecondsPerTicket));
  const totalHoursDecimal = totalSeconds / 3600;
  const averagePerDayDecimal = totalHoursDecimal / 24 / processedHours.length;

  document.getElementById("totalHours").textContent = formatTime(
    hours,
    minutes,
    seconds,
  );
  ...
  document.getElementById("averageTime").textContent = formatTime(
    avgHours,
    avgMinutes,
    avgSeconds,
  );
  document.getElementById("averagePerDay").textContent =
    averagePerDayDecimal.toFixed(4) + " días";
...
function restrictInputFormat(textarea) {
  ...
  if (!validPattern.test(value)) {
    ...
    showInputError(
      "Solo números, : , . , comas y saltos de línea son permitidos",
    );
  } else if (!validateFullInput(value)) {
    // Si los caracteres son válidos pero el formato no lo es
    textarea.value = textarea.dataset.lastValid || "";
    showInputError(
      "Formato inválido. Usa HH:MM:SS, HH:MM, o horas (ej. 8, 7.5)",
    );
  } else {
